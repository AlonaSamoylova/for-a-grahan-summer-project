...
def CalcMSD(folder_path, min_length=200, time_ratio=2, seg_size=10): #enlarge min length -> 100, 200, van hoff corelation function =>top 10-20 of longest traj
    ...
    """
    global metrics  # ensure metrics dict here is the same one used later for master_df
...
if missing_cols:
    ...
    # allow pipeline to continue: fill missing label columns with defaults
    if "is_hopper" not in master_df.columns:
        master_df["is_hopper"] = 0
    if "is_alpha_fast" not in master_df.columns:
        master_df["is_alpha_fast"] = 0

if "msd_mag_1s" in master_df.columns:
    master_df = master_df[
